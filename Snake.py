...
pygame.display.set_caption("Snake - Nawel Asgar")
...
#             pygame.draw.rect(win, WHITE2, rect, 1)

# def draw_hamiltonian_path(path):
#     for i, (x, y) in enumerate(path):
#         # Draw nodes
#         center = (x * tile_size + tile_size // 2, y * tile_size + tile_size // 2)
#         pygame.draw.circle(win, GREEN, center, 5)
        
#         # Draw edges
#         if i < len(path) - 1:
#             next_center = (path[i+1][0] * tile_size + tile_size // 2, path[i+1][1] * tile_size + tile_size // 2)
#             pygame.draw.line(win, WHITE2, center, next_center, 2)
        
#         # Display vertex numbers
#         font = pygame.font.SysFont("Arial", 12)
#         text = font.render(str(i), True, WHITE2)
#         win.blit(text, (center[0] + 5, center[1] + 5))


# The draw_background function is used to draw onto the window the background of the game.
def draw_background():
    for x in range(w):
        for y in range(h):
            if (x + y) % 2 == 1:
...
def draw_snake(snake):
    for position in snake.segments:
        ...
        pygame.draw.rect(win, WHITE1, snake_rect)
        # pygame.draw.rect(win, WHITE2, snake_rect, tile_size // 10)
...
def draw_food(food_pos):
    ...
    pygame.draw.rect(win, RED, food_rect, tile_size // 10)
...
# This function is also responsible for extending the snake's body whenever it eats food.
...
def game_over_message():
    ...
    font = pygame.font.SysFont("arial", win_height // 7)
...
def game():
    ...
    running = True
    # Video capture setup
    frame_size = (win.get_width(), win.get_height())
    video = cv2.VideoWriter('gameplay_fast.mp4', cv2.VideoWriter_fourcc(*'mp4v'), video_fps, frame_size)
    frame_skip = 4  # Skip every 4 frames to achieve 4x speed
    frame_count = 0
